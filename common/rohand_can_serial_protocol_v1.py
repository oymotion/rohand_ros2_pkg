...
# CAN transport wrapper for python-can / Peak
class PcanTransport:
    """
    PCAN wrapper that mimics a serial-like object:
      - write(bytes)
      - in_waiting (property)
      - read(n)
      - close()
    """
    def __init__(self, hand_id=[2, 3], master_id=[1, 4], port_num=1, baudrate=1000000):
        # buss type 'pcan' for PCAN hardware
        self.bus = can.interface.Bus(interface='pcan', channel=f"PCAN_USBBUS{(port_num)}", bitrate=baudrate)
        # self.bus = can.interface.Bus(interface='socketcan', channel=f"can{int(port_num)}", bitrate=baudrate)
        self._rxq = {}
        for device_id in hand_id:
            self._rxq[device_id] = deque()
        self._running = True
        self._thread = threading.Thread(target=self._reader, daemon=True)
        self._thread.start()
        self._addr = hand_id
        self._addr_master = master_id
    ...
    def _reader(self):
        while self._running:
            ...
            for i in range(len(self._addr_master)):
                if msg.arbitration_id != self._addr_master[i]:
                    continue
                for b in msg.data:
                    self._rxq[self._addr[i]].append(b)
    ...
    def write(self, data):
        ...
        while pos < len(data):
            chunk = bytes(data[pos:pos+8])
            for addr in self._addr:
                try:
                    msg = can.Message(arbitration_id=int(addr), is_extended_id=False, data=chunk)
                    self.bus.send(msg)
                except Exception:
                    pass
            pos += 8
    ...
    def read(self, hand_id, n=1):
        out = bytearray()
        for _ in range(min(n, len(self._rxq[hand_id]))):
            out.append(self._rxq[hand_id].popleft())
        return bytes(out)
...
HAND_CMD_GET_FINGER_CURRENT_LIMIT = 0x05  # Get motor current limit of finger
HAND_CMD_GET_FINGER_CURRENT       = 0x06  # Get motor current of finger
HAND_CMD_GET_FINGER_FORCE_TARGET  = 0x07  # Get target force of finger
HAND_CMD_GET_FINGER_FORCE         = 0x08  # Get force of finger
HAND_CMD_GET_FINGER_POS_LIMIT     = 0x09  # Get absolute position limit of finger
HAND_CMD_GET_FINGER_POS_ABS       = 0x0A  # Get current absolute position of finger
HAND_CMD_GET_FINGER_POS           = 0x0B  # Get current logical position of finger
...
HAND_CMD_GET_FINGER_POS_ABS_ALL   = 0x0E  # Get current absolute position of all fingers
HAND_CMD_GET_FINGER_POS_ALL       = 0x0F  # Get current logical position of all fingers
HAND_CMD_GET_FINGER_ANGLE_ALL     = 0x10  # Get current angle of all fingers
...
HAND_CMD_GET_VENDOR_ID            = 0x3F  # Get vendor id
...
HAND_CMD_SET_FINGER_FORCE_TARGET  = 0x47  # Set target force of finger
...
HAND_CMD_RESET_FORCE              = 0x54  # 0xReset force
...
class OHandContext:
    ...
    def send_cmd(self, addr, addr_master, cmd, data=None):
        """
    ...
    def get_response(self, addr, addr_master, cmd, time_out, resp_bytes):
        """
        Send a command to the hand with the given id, wait for the response and store it in the same data buffer,
        and return the error code and the remote error.
        ...
        while not self.is_whole_packet:
            ...
            data_bytes = self.serial_port.read(addr, self.serial_port.in_waiting)
            for ch in data_bytes:
                self.on_data(ch, addr_master)
    ...
    def on_data(self, data, addr_master):
        """
        ...
        if self.decode_state == WAIT_ON_HEADER_0:
            ...
            if self.packet_data[NODE_ID_BYTE_NUM] == addr_master:
...
class OHandProtocol:
    ...
    def get_protocol_version(self, hand_id, addr_master):
        """
        ...
        err = self.ctx.send_cmd(hand_id, addr_master, HAND_CMD_GET_PROTOCOL_VERSION)

        if err == HAND_RESP_SUCCESS:
            ver = bytearray()  # To hold the major version
            err, remote_err = self.ctx.get_response(hand_id, addr_master, HAND_CMD_GET_PROTOCOL_VERSION, self.ctx.timeout, ver)
    ...
    def get_hardware_version(self, hand_id, addr_master):
        """
        Get the hardware version of the hand with the given id.
        ...
        err = self.ctx.send_cmd(hand_id, addr_master, HAND_CMD_GET_HW_VERSION)

        if err == HAND_RESP_SUCCESS:
            ver = []
            err, remote_err = self.ctx.get_response(hand_id, addr_master, HAND_CMD_GET_HW_VERSION, self.ctx.timeout, ver)

            if err == HAND_RESP_SUCCESS:
                hw_type = ver[0]
                hw_ver = ver[1]
                boot_ver = (ver[2] << 8) | ver[3]

        self.ctx.release_lock()

        return err, remote_err, hw_type, hw_ver, boot_ver
    
    def set_custom(self, hand_id, addr_master, speed=None, position=None, angle=None, get_flag=0x00):
        """
        Send a custom command to the hand with the given id.

        :param hand_id: address of the hand to send the command to
        :param addr_master: address of the host
        ...
        err = self.ctx.send_cmd(hand_id, addr_master, HAND_CMD_SET_CUSTOM, data)

        if err == HAND_RESP_SUCCESS:
            err, remote_err = self.ctx.get_response(hand_id, addr_master, HAND_CMD_SET_CUSTOM, self.ctx.timeout, data)
            if err != HAND_RESP_SUCCESS:
                print("\033[31m[ERROR]  hand timeout: {}\033[0m".format(hand_id))
