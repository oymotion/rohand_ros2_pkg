...
class OHandProtocol:
    ...
    def get_hardware_version(self, hand_id):
        """
        ...
        err = self.ctx.send_cmd(hand_id, HAND_CMD_GET_HW_VERSION)

        if err == HAND_RESP_SUCCESS:
            ver = []
            err, remote_err = self.ctx.get_response(hand_id, HAND_CMD_GET_HW_VERSION, self.ctx.timeout, ver)
    ...
    def reset_force(self, hand_id):
        """
        Reset force, that is, calibrate.
        ...
        err = self.ctx.send_cmd(hand_id, HAND_CMD_RESET_FORCE)

        if err == HAND_RESP_SUCCESS:
            err, remote_err = self.ctx.get_response(hand_id, HAND_CMD_RESET_FORCE, self.ctx.timeout, data)
