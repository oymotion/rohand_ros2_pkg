    ...
    def __init__(self):
        ...
        for i in range(10):
            ...
            for hand_id in self.hand_ids_:
                ...
                try:
                    err, remote_err, hw_type, hw_ver, boot_ver = self.rohand_protocol.get_hardware_version(hand_id)
                except Exception as exc:
                    traceback.print_exc()
                    self.get_logger().error(f"exception occurred when calling get_hardware_version(): {exc}")

                if err != HAND_RESP_SUCCESS:
                    self.get_logger().error(f"get_hardware_version() returned an error: {err}")
                else:
                    hard_ware_type = (hw_type << 8) | hw_ver

                    if(hard_ware_type == ROH_HARDWARE_TYPE):
                        self.get_logger().info("Force sensor supported.")
                        # Reset force
                        try:
                            err = self.rohand_protocol.reset_force(hand_id)
                        except Exception as exc:
                            traceback.print_exc()
                            self.get_logger().error(f"exception occurred when calling reset_force(): {exc}")
